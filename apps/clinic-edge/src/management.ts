...
import { listOpenConflicts } from './conflict.ts';
...
export async function dashboard(pool: Pool, asOf: string): Promise<Dashboard> {
  const [ageing, alerts, critical, pendingSync, charge, openConflicts] = await Promise.all([
  ...
  if (openConflicts.length > 0) exceptions.push({ type: 'sync_conflicts', label: 'Sync conflicts awaiting human resolution', count: openConflicts.length, queue: '/api/sync/conflicts', owner: 'System administrator' });
...
export async function resolveSiteScope(
  ...
  const r = await pool.query(`SELECT id FROM organisation.site WHERE active=true`);
  const allSites = r.rows.map((x) => x.id as string);
  return effectiveSiteFilter(args.roles, args.userSite, args.requestedSites ?? [], allSites);
}

/**
 * Gate a drill-through from a summary KPI to underlying detail (MGT-006). A
 * summary is never a back door: reaching patient/clinical detail requires the
 * clinical-detail permission regardless of dashboard access. Denials are audited.
 */
export async function drillThrough(
  ...
  if (!canDrill(args.roles, args.target)) {
    await pool.query(
      `INSERT INTO audit.audit_event (id, actor_user, action, resource_type, outcome, reason, captured_at, event_hash)
       VALUES ($1,$2,'access','management_drill','deny',$3, now(), $4)`,
      [uuidv7(), args.actor ?? null, `drill to ${args.target} requires ${permission}`, 'mgmt-drill:' + uuidv7()],
    );
...
export async function addCommentary(
  ...
  const id = uuidv7();
  await pool.query(
    `INSERT INTO organisation.kpi_commentary (id, kpi_id, period, commentary, action, action_owner, due_date, authored_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [id, args.kpiId, args.period, args.commentary, args.action ?? null, args.actionOwner ?? null, args.dueDate ?? null, args.authoredBy ?? null],
  );
  return { id };
}

/** Commentary history for a KPI period, newest first (MGT-010). */
export async function listCommentary(pool: Pool, args: { kpiId: string; period: string }): Promise<Commentary[]> {
  const r = await pool.query(
    `SELECT id, kpi_id, period, commentary, action, action_owner, to_char(due_date,'YYYY-MM-DD') AS due_date, status, authored_by,
            to_char(authored_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"') AS authored_at
     FROM organisation.kpi_commentary WHERE kpi_id=$1 AND period=$2 ORDER BY authored_at DESC`,
    [args.kpiId, args.period],
  );
  return r.rows.map((x) => ({
    id: x.id,
    kpiId: x.kpi_id,
    ...
    action: x.action,
    actionOwner: x.action_owner,
    dueDate: x.due_date ?? null,
    status: x.status,
    authoredBy: x.authored_by,
    authoredAt: x.authored_at,
  }));
}
